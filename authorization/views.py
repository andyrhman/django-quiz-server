...
class LoginAPIView(APIView):
    def post(self, request):
        ...
        access_max_age = int((getattr(settings, 'SIMPLE_JWT', {}).get('ACCESS_TOKEN_LIFETIME', datetime.timedelta(minutes=1))).total_seconds())
        refresh_max_age = int((getattr(settings, 'SIMPLE_JWT', {}).get('REFRESH_TOKEN_LIFETIME', datetime.timedelta(minutes=2))).total_seconds())
